...
// options is the resolved option set. It is separate from Source because New
// needs the policy BEFORE the Source exists: the per-message response cap it
// installs on the HTTP transport is derived from attachments.max_message_bytes.
type options struct {
	pol *policy.Policy
...
// nil selects policy.Default(). Build the policy once per run with
// config.Config.Policy and share it across connectors.
...
// need to drive the free-space floor without filling a real disk; production
// leaves it alone.
...
// refusing every attachment — the floor exists to protect a nearly-full disk,
// not to stop archiving whenever statfs is unavailable.
